...
import ge.model.ModelNiveau;
import ge.repository.RepositoryNiveau;
...
public class ControllerNiveau {
	...
	private final RepositoryNiveau repository;
	ResponseHandler responseHandler = new ResponseHandler();

	ControllerNiveau(RepositoryNiveau repository) {
		this.repository = repository;
	}

	@GetMapping("/allNiveau")
	...
	ResponseEntity<Object> add(@RequestBody ModelNiveau model) {
		try {
	...
	ResponseEntity<Object> update(@RequestBody ModelNiveau model) {
		try {
			return responseHandler.generateResponse(HttpStatus.OK,
					repository.findById(model.getIdNiveau()).map(newModel -> {
						newModel.setCodeNiveau(model.getCodeNiveau());
						newModel.setLibelleNiveau(model.getLibelleNiveau());
